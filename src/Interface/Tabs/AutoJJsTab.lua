...
function Tab:Render(WindowTab, Hub, Config, State)
    ...
    -- ESSENCIAIS
    -- ============================
    WindowTab:Section({ Title = "Essenciais", Desc = "Configuração geral e contagem.", Icon = "list-ordered" })

    local Status = WindowTab:Section({ Title = "Status", Desc = "Parado." })
    ...
    WindowTab:Input({ Flag = "JJsInicial", Title = "Inicial", Desc = "Número de início da contagem.", Value = tostring(Cfg.Inicial or 1), Callback = function(v) Cfg.Inicial = tonumber(v) or 1 end })
    WindowTab:Input({ Flag = "JJsFinal", Title = "Final", Desc = "Número de término da contagem.", Value = tostring(Cfg.Final or 100), Callback = function(v) Cfg.Final = tonumber(v) or 100 end })

    WindowTab:Toggle({ Flag = "JJsPular", Title = "Pular", Icon = "arrow-up", Desc = "Faz o avatar pular fisicamente a cada número enviado.", Value = Cfg.Pular == true, Callback = function(v) Cfg.Pular = v end })
    WindowTab:Toggle({ Flag = "JJsEspaco", Title = "Espaçamento", Desc = "Insere um espaço entre o número e o sufixo (\"1 !\" em vez de \"1!\").", Value = Cfg.Espacamento == true, Callback = function(v) Cfg.Espacamento = v end })

    -- ============================
    -- FORMATAÇÃO DE TEXTO
    -- ============================
    WindowTab:Section({ Title = "Formatação de Texto", Desc = "O que aparece depois de cada número.", Icon = "type" })
    ...
    -- ============================
    WindowTab:Section({ Title = "Intervalo", Desc = "Escolha só um: inteligente, fixo ou dinâmico.", Icon = "timer" })
    ...
    WindowTab:Input({ Flag = "JJsIntElTempo", Title = "Tempo (segundos)", Desc = "Duração total da sequência inteira.", Value = tostring(Cfg.IntervaloInteligenteTempo or 60), Callback = function(v) Cfg.IntervaloInteligenteTempo = tonumber(v) or 60 end })

    IntFXToggle = WindowTab:Toggle({
    ...
    WindowTab:Slider({ Flag = "JJsIntFxVal", Title = "Intervalo fixo (segundos)", Desc = "Tempo exato entre mensagens.", Step = 0.1, Value = { Min = 0.2, Max = 10, Default = Cfg.IntervaloFixoValor or 1.4 }, Callback = function(v) Cfg.IntervaloFixoValor = v end })

    IntDYToggle = WindowTab:Toggle({
    ...
    WindowTab:Input({ Flag = "JJsIntDyMin", Title = "Valor mínimo (segundos)", Desc = "Menor tempo de espera possível.", Value = tostring(Cfg.IntervaloDinamicoMin or 1), Callback = function(v) Cfg.IntervaloDinamicoMin = tonumber(v) or 1 end })
    WindowTab:Input({ Flag = "JJsIntDyMax", Title = "Valor máximo (segundos)", Desc = "Maior tempo de espera possível.", Value = tostring(Cfg.IntervaloDinamicoMax or 3), Callback = function(v) Cfg.IntervaloDinamicoMax = tonumber(v) or 3 end })

    -- ============================
    -- EXTRAS
    -- ============================
    WindowTab:Section({ Title = "Extras", Desc = "Ajustes adicionais.", Icon = "sparkles" })
    WindowTab:Toggle({ Flag = "JJsReverso", Title = "Modo reverso", Icon = "rotate-ccw", Desc = "Inverte a contagem: começa no Final e desce até o Inicial.", Value = Cfg.ModoReverso == true, Callback = function(v) Cfg.ModoReverso = v end })
end
