...
function Tab:Render(WindowTab, Hub, Config)
    ...
    -- Heartbeat leve só pra atualizar o texto (não é o loop de mira em si,
    -- que já roda no próprio Aimbot.lua). Usa a mesma flag global que o
    -- resto do hub usa pra sinalizar "hub carregado", senão esse loop
    -- ficaria rodando pra sempre mesmo depois de fechar o hub.
    task.spawn(function()
    ...
    WindowTab:Dropdown({
        ...
        Values = { "Mais perto da mira", "Menor vida" },
    ...
    WindowTab:Slider({
        ...
        Callback = function(v) Aim.Settings.Smoothness = v end
    })
    WindowTab:Slider({
        Flag = "AimF", Title = "Raio do FOV", Step = 1,
    ...
    WindowTab:Slider({
        ...
        Callback = function(v) Aim.Settings.HitboxSize = v end
    })
end
