...
table.insert(Connections, RunService.RenderStepped:Connect(function()
    ...
    if PlayerMods.Settings.Fly and root and FlyBV then
        ...
        local flyDir = moveDir + Vector3.new(0, vertical, 0)
        FlyBV.Velocity = flyDir * PlayerMods.Settings.FlySpeed

        -- Gira o personagem pra encarar a direção do voo (tipo avião) —
        -- sempre ativo, não é opcional: sem isso o char fica "arrastando"
        -- de lado/costas enquanto voa, porque o PlatformStand desliga o
        -- auto-rotate normal do Humanoid.
        if flyDir.Magnitude > 0.05 then
            root.CFrame = CFrame.new(root.Position, root.Position + flyDir)
        end
    end
